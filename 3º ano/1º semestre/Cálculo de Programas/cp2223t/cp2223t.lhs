...
\\Decidímos que seria benéfico simplificar a expressão coef, tal como se pode ver abaixo no código.
  \\\texttt{<<h,g>,f>} = (|<<[1,coef],[1,p1.p1]>,[0,p2.p1]>|)
  \\\texttt{<<h,g>,f>} = (|<[<1,1>,<coef,p1.p1>],[0,p2.p1]>|)
  \\\texttt{<<h,g>,f>} = (|[<<1,1>,0>,<<coef,p1.p1>,p2.p1>]|)  
\\Resultando em:
  \\\texttt{<<h,g>,f>} = for \texttt{<<coef,p1.p1>,p2.p1>>((1,1),0)}

Assim sendo, construímos o seguinte código:
\begin{code}
loop a b c = split (split (coef a b c) (p1.p1)) (p2.p1)
  where coef a b c ((x,y),z) = a * x + b * y + z * c
initial = ((1,1),0)
wrap = p2
...
gene          = (id -|- id >< f) . out 
  where
    f         = anaList geneAna . map (drop 4)
    geneAna   = (id -|- l) . outList
    l(h,t)    = (h:take (k t) t , drop (k t) t)
    k         = cataList (either (const 0) h) 
    h(c,ls)   = if take 1 c == " " then 1 + ls else 0
...
\xymatrix{
  |Exp S S| & & S + S \times (|Exp S S|)^*\ar[ll]_{|inExp|} \\
  S^*\ar@@/_1.5pc/[rr]_{|gene|}\ar[r]^(0.35){|out|}\ar[u]^{|tax|} & S + S \times S^*\ar[r]^(0.45){id + id \times f} & S + S \times (S^*)^*\ar[u]_{id + id \times tax^*}
}
\end{eqnarray*}
...
\begin{eqnarray*}
\xymatrix{
  {(S^*)^*} & & 1 + (S^*) \times (S^*)^*\ar[ll]_{|inList|} \\
  S^*\ar@@/_1.5pc/[rr]_{|geneAna|}\ar[r]^(0.45){|outList|}\ar[u]^{|f|} & 1 + S \times S^*\ar[r]^(0.45){id + l} & 1 + S^* \times (S^*)^*\ar[u]_{id + id \times f^*}
}
\end{eqnarray*}
...
\begin{eqnarray*}
\xymatrix{
  {S^*}\ar[d]^{|k|}\ar[rr]^{|outList|} & & 1 + S \times S^*\ar[d]^{id + id \times k} \\
  Int & & 1 + S \times Int\ar[ll]_{|[const 0 , h]|}
}
\end{eqnarray*}
...
\begin{code}
post          = cataExp (either g1 g2) where
  g1          = singl . singl
  g2(h,resto) = [[h]] ++ (map (h:) (concL(resto))) 
  concL       = cataList (either nil conc)
\end{code}

Este diagrama mostra-nos o gene do catamorfismo de arvores de expressão para gerar todos os caminhos possíveis na arvore de expressão. Para isso temos um catamorfismo de listas com o seguinte gene: [g1,g2]
onde g1 retorna uma lista de listas com o elemento injetado. E o g2 obtem todos os caminhos percorrendo a arvore de expressão e obtendo como resultado uma concatenação  sucessiva da cabeça à lista de listas.

\begin{eqnarray*}
\xymatrix{
  {|Exp S S|}\ar[d]^{|post|}\ar[rr]^{|outExp|} & & S + S \times (Exp S S)^*\ar[d]^{id + id \times post^*} \\
  {(S^*)^*} & & S + S \times {(S^*)^*}\ar[ll]^{|[g1,g2]|}
}
\end{eqnarray*}
Este diagrama mostra-nos o gene do catamorfismo que concatena listas de listas em listas .

\begin{eqnarray*}
\xymatrix{
  {(S^*)^*}\ar[d]^{|concL|}\ar[rr]^{|outList|} & & 1 + (S^*) \times (S^*)^*\ar[d]^{id + id \times concL^*} \\
  {(S^*)}& & 1 + (S^*) \times {(S^*)^*}\ar[ll]^{|[nil,conc]|}
}
\end{eqnarray*}
\subsection*{Problema 3}
\begin{code}
...
gsq (sq,0) = (principal sq, [])
gsq (sq,n) = (principal sq, zip (divide sq) (replicate 8 (n-1)))
principal :: Square -> Square
principal ((x,y),s) = let sq =  s/3 in ((x+sq,y+sq),sq) 

divide :: Square -> [Square]
divide ((x,y),s) = [((x+i*s/3, y+j*s/3), s/3) | i <- [0..2], j <- [0..2], not (i==1 && j==1)] 
-- divide os squares evitando o principal i==1 && j==1
...
carpets = anaList gcarpets
   
          
gcarpets = (id -|-  split f id) . outNat
         where f a =  sierpinski (((0,0),32),a)
...
Definição da função squares. Que pega num Square e num int e gera os squares à volta , através de um anamorfismo de Rose Trees. O gene deste anamorfismo 
usa o quadrado passado por argumento e depois vai gerando os quadrados à volta tendo em conta a profundidade. O anamorfismo gera assim os quadrados todos à volta para uma determinada profundidade.

\begin{eqnarray*}
\xymatrix{
  {|Rose Square|} & & Square \times (|Rose Square|)^*\ar[ll]_{|inRose|} \\
  {Square \times Int}\ar[rr]_{|gsq|}\ar[u]^{|squares|} && {Square \times (Square\times Int)^*}\ar[u]_{id \times squares^*}
}
\end{eqnarray*}

Definição da função rose2List. Que pega numa Rose Tree e transforma numa lista. Através do catamorfismo de Rose Trees. O gene basicamente faz a concatenação de listas de listas em listas.
\begin{eqnarray*}
\xymatrix{
  {|Rose Square|}\ar[d]^{|rose2List|}\ar[rr]^{|outRose|} & & Square \times (|Rose Square|)^*\ar[d]^{id \times (rose2List)^*} \\
  {Square^*}& & Square \times (Square^*)^*\ar[ll]^{cons.(id \times concat)}
}
\end{eqnarray*}


Definição do hilomorfismo de sierpinski. Combinação do anamorfismo de Rose Trees com o catamorfismo de rose trees que nos permite gerar as imagens do sierpinski.
\begin{eqnarray*}
\xymatrix{
  {Square \times Int}\ar[rr]_{|gsq|}\ar[d]^{|squares|} && {Square \times (Square\times Int)^*}\ar[d]_{id \times squares^*}\\
  {|Rose Square|}\ar[d]_{|rose2List|}\ar@@/_1.5pc/[rr]_{|outRose|} & & Square \times (|Rose Square|)^*\ar[d]_{id \times (rose2List)^*}\ar@@/_1.5pc/[ll]_{|inRose|}\\
  {Square^*}& & Square \times (Square^*)^*\ar[ll]^{cons.(id \times concat)} 
}
\end{eqnarray*}

Definição da função carpets através de um anamorfismo de listas. Recebemos um inteiro como argumento e geramos Listas de listas de Squares para N profundidade, sendo que cada sublista tem os squares até um nivel de profundidade.

\begin{eqnarray*}
\xymatrix{
  {(Square^*)^*} & & 1+ Square^* \times (Square^*)^*\ar[ll]_{|inList|} \\
  {Int}\ar[rr]_{|gcarpets|}\ar[u]^{|carpets|} && {1 + Square^* \times Int}\ar[u]_{id + id \times carpets^*}
}
\end{eqnarray*}
